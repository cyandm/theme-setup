<div class="single-blog-post">
	...
	<div class="block md:flex justify-between items-center">
		...
		<div class="flex flex-row justify-center items-end p-3 gap-x-3 mt-3">
			<div
				 class="w-24 md:w-12 h-16 flex flex-col justify-center items-center bg-[#3F3CC30F] rounded-xl border border-zinc-700 gap-y-2 py-2">
			...
			<div class="comment-container w-24 md:w-12 h-16 flex flex-col justify-center items-center bg-[#3F3CC30F] rounded-xl border border-zinc-700 gap-y-2 py-2"
				...
				<a href="#comments-section"
				   class="comment-icon cursor-pointer flex flex-col items-center">
					<svg xmlns="http://www.w3.org/2000/svg"
						 fill="none"
						 viewBox="0 0 24 24"
						 stroke-width="1.5"
			...
			<div class="like-container w-24 md:w-12 h-16 flex flex-col justify-center items-center bg-[#3F3CC30F] rounded-xl border border-zinc-700 gap-y-2 py-2"
				...
				<!-- gray-icon-->
				<span
					  class="like-icon like-icon-empty cursor-pointer <?php echo in_array( [ 'guest_user_id' ] ?? '', get_post_meta( get_the_ID(), 'liked_users', true ) ?: [] ) ? 'hidden' : ''; ?>">
					<svg xmlns="http://www.w3.org/2000/svg"
						 fill="none"
						 viewBox="0 0 24 24"
						 stroke-width="1.5"
				...
				<!--red-icon-->
				<span
					  class="like-icon like-icon-red cursor-pointer hidden <?php echo in_array( [ 'guest_user_id' ] ?? '', get_post_meta( get_the_ID(), 'liked_users', true ) ?: [] ) ? '' : 'hidden'; ?>">
					<svg xmlns="http://www.w3.org/2000/svg"
						 fill="red"
						 viewBox="0 0 24 24"
						 stroke-width="1.5"
						 stroke="currentColor"
						 class="size-5 text-red-800">
						<path stroke-linecap="round"
							  stroke-linejoin="round"
							  d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12Z" />
					</svg>
				...
				<span
					  class="like-count text-sm"><?php echo get_post_meta( get_the_ID(), 'like_count', true ) ?: 0; ?></span>
			</div>
		</div>

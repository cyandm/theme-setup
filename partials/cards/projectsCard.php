...
for ( $i = 1; $i < 5; $i++ ) {
	...
	if ( ! $project_image = get_field( 'project_images_' . $i ) )
		continue;
...
<div style="--start-color: <?php echo $section_color ?>;"
	...
	<div class="container mx-auto flex-row group-even:flex-row-reverse flex  max-md:!flex-col-reverse pt-8">
		<div class="flex flex-col gap-y-5">
			...
			<div class="flex justify-end">
				<a href="<?php echo get_permalink() ?>"
				   class="btn-primary rounded-3xl items-center px-5 py-2 ">
					<?php _e( 'اطلاعات بیشتر', 'cyn-dm' ) ?>
				</a>
			</div>

			<span class="text-2xl font-bold">
				<?php _e( 'خدمات ما', 'cyn-dm' ) ?>
			</span>
		...
		<div class="flex flex-col">
			...
			<div class="flex gap-2 justify-center">
				<button id="swiper-button-next-<?php echo get_the_ID() ?>"
						class=" flex border border-neutral rounded-full py-2 px-2 justify-center items-center disabled:opacity-20 transition-opacity">
					<span class="size-5 -scale-100">
						<?php Icon::print( 'Left-1' ) ?>
					</span>
				</button>

				<button id="swiper-button-prev-<?php echo get_the_ID() ?>"
						class=" flex border border-neutral rounded-full py-2 px-2 justify-center items-center disabled:opacity-20 transition-opacity">
					<span class="size-5">
						<?php Icon::print( 'Left-1' ) ?>
					</span>

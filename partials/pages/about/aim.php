...
<section class="relative container mt-8 isolate"
	...
	<div
		 class="bg-white bg-opacity-5 bg-transparent backdrop-blur-md rounded-2xl  flex flex-col justify-center items-center gap-y-4 pt-8">
		...
		<swiper-container slides-per-view="1"
			...
			<?php foreach ( $aim_sections as $aim_section ) : ?>
				<swiper-slide class="swiper-slide transition-opacity duration-500 ease-in-out">
					...
					<div class="text-base md:text-xl md:leading-9 flex text-justify leading-9 pt-4">
						<?php echo $aim_section['aim-text'] ?>
					</div>
				</swiper-slide>
			<?php endforeach; ?>
		</swiper-container>
		...
		<div class="grid grid-cols-2 z-10 gap-2 p-4 justify-center">
			<button
					class="swiper-button-next flex border border-neutral rounded-3xl py-2 pr-4 pl-5 justify-center items-center">
				<span class="size-5">
			...
			<button
					class="swiper-button-prev flex border border-neutral rounded-3xl py-2 pr-5 pl-4 justify-center items-center hover:text-neutral-50">
				<span class="text-sm">
					<?php _e( 'قبلی', 'cyn-dm' ) ?>
				</span>
				<span class="size-5">
